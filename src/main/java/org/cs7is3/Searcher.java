...
public class Searcher {
    ...
    private static final String RUN_TAG = "CS7IS3_LMDirichlet_English";
    ...
    public void searchTopics(Path indexPath, Path topicsPath, Path outputRun, int numDocs) throws IOException {
        ...
        searcher.setSimilarity(new LMDirichletSimilarity()); // or new LMDirichletSimilarity(2500f);
        ...
        try (PrintWriter writer = new PrintWriter(outputRun.toFile())) {
            for (Topic topic : topics) {
                try {
                    // Build query: title + description (和你之前 A2 一样的风格)
            ...
            System.out.println("Search Complete (LMDirichlet + EnglishAnalyzer).");
        } finally {
            reader.close();
        }
    }
