...
public class Searcher {
        ...
        try (PrintWriter writer = new PrintWriter(outputRun.toFile())) {
            for (Topic topic : topics) {
                try {
                    ...
                    if (topic.title != null)
                        anchor.add(new BoostQuery(parser.parse(QueryParser.escape(topic.title)), 3.0f),
                                   BooleanClause.Occur.SHOULD);
                    if (topic.description != null)
                        anchor.add(new BoostQuery(parser.parse(QueryParser.escape(topic.description)), 1.3f),
                                   BooleanClause.Occur.SHOULD);
                    if (topic.narrative != null) {
                        String n = filterNarrative(topic.narrative);
                        if (!n.isEmpty())
                            anchor.add(new BoostQuery(parser.parse(QueryParser.escape(n)), 0.5f),
                                       BooleanClause.Occur.SHOULD);
                    }
                    ...
                    if (!exp.isEmpty()) {
                        finalQ.add(
                                new BoostQuery(
                                        parser.parse(QueryParser.escape(String.join(" ", exp))),
                                        0.5f
                                ),
                                BooleanClause.Occur.SHOULD
                        );
                    ...
                    for (int i = 0; i < hits.length; i++) {
                        writer.printf(
                                "%s Q0 %s %d %.4f %s%n",
                                topic.number,
                                searcher.doc(hits[i].doc).get("DOCNO"),
                                i + 1,
                                hits[i].score,
                                RUN_TAG
                        );
    ...
    private Map<String, Boolean> analyze(String text) throws IOException {
        ...
        for (int i = 0; i < max; i++) {
            if (raw[i].length() > 0 && Character.isUpperCase(raw[i].charAt(0))) {
                caps.add(raw[i].replaceAll("[^a-zA-Z]", "").toLowerCase());
            }
        }
    ...
    private Set<String> getQueryTerms(Topic t) throws IOException {
        ...
        String q = (t.title != null ? t.title : "") + " " + (t.description != null ? t.description : "");
        try (TokenStream ts = analyzer.tokenStream("TEXT", new StringReader(q))) {
            CharTermAttribute term = ts.addAttribute(CharTermAttribute.class);
            ts.reset();
